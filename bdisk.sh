...
disco=$(fdisk -l | grep -E "/dev/hd[a-z]:|/dev/sd[a-z]:|/dev/nvme0n[1-9]:" | awk '{print $2,$3,$4}' | tr ":" "\t" | sed 's/,$//' | fzy --prompt="Selecciona el disco al que quieres hacerle un backup --> " | awk '{print $1}')
...
num_part=$(lsblk -lf $disco | grep -E "$disco_f([1-9]|[1-8][0-9]|9[0-9]|1[01][0-9]|12[0-8])|${disco_f}p([1-9]|[1-8][0-9]|9[0-9]|1[01][0-9]|12[0-8])" | wc -l)
mapfile -t part < <(lsblk -lf $disco | grep -E "$disco_f([1-9]|[1-8][0-9]|9[0-9]|1[01][0-9]|12[0-8])|${disco_f}p([1-9]|[1-8][0-9]|9[0-9]|1[01][0-9]|12[0-8])" | awk '{print $1}' )
mapfile -t tipo_part < <(lsblk -lf $disco | grep -E "$disco_f([1-9]|[1-8][0-9]|9[0-9]|1[01][0-9]|12[0-8])|${disco_f}p([1-9]|[1-8][0-9]|9[0-9]|1[01][0-9]|12[0-8])" | awk '{print $2}')
...
if [ $aviso_montaje != 0 ]; then
    ...
    if [[ "$user" == "S" || "$user" == "s" || "$user" == "" ]]; then
        ...
        dir_user=$(find /isodevice /home /mnt /run /media -type d 2>/dev/null | fzy --prompt "Escoge un directorio para guardar las copias, si no existe se creara --> ")_$(date -I)
